...
- English identifiers; UI strings are Turkish literals used as keys in `Timelapse/Localizable.xcstrings` (12 languages: tr source + ar de en es fr hi ja ko pt ru zh-Hans). **Every new user-facing string needs translations in all 11 target languages** — pattern: a Python script that inserts `localizations.<lang>.stringUnit` entries (see git history). `String(localized:bundle:.appLanguage)` in Swift code; plain `LocalizedStringKey` Text works too. **The widget extension now has its own catalog** (`Widgets/Localizable.xcstrings`, same 12 languages) — keep it in sync for widget strings.
...
- `Timelapse/MainTabView.swift` — tab shell, glass bar, deep link (`flapse://capture`, entry-limit gated)
